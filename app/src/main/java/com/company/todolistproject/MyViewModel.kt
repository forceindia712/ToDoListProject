...
import com.company.todolistproject.FileHelper
import com.company.todolistproject.MyItem
...
class MyViewModel(private val sp: SharedPreferences) : ViewModel() {
    ...
    fun removeItem(item: MyItem, position: Int) {
        itemlist.postValue(FileHelper.removeItem(getItemList(), position))
        writeItemList()
    ...
    fun changeViewList(position: Int) {
        itemlist.postValue(FileHelper.renewItem(getItemList(), position))
        writeItemList()
        if (allRemoved()) {
            visibleDeleted.postValue(false)
            updateItemListNotDeleted()
